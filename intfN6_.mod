: $Id: intf.mod,v 1.794 2009/01/07 18:50:03 billl Exp $
...
PROCEDURE jitcon (tm) {
  ...
  if (jrj<jrmax) { 
    ...
    jrj++;
  } else if (wf2 && jrmax) { // saving spike times
    fprintf(wf1,"//b9 -2 t%0.2f %ld %ld\n",t/1e3,jrmax,ftell(wf2));
    fwrite(jrid,sizeof(double),jrmax,wf2); // write id
    fwrite(jrtv,sizeof(double),jrmax,wf2); // write times
    jrj=0;
  }
...
:** vers gives version
PROCEDURE vers () {
  printf("$Id: intf.mod,v 1.794 2009/01/07 18:50:03 billl Exp $\n")
}
